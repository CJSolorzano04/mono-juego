...
const imagePaths = {
    ...
    run1: 'assets/01.png', run2: 'assets/02.png',
...
for (let key in imagePaths) {
    ...
    images[key].onload = () => {
        imagesLoaded++;
        if (imagesLoaded === Object.keys(imagePaths).length) init();
    };
}
...
class Monkey {
   ...
   update() {
        if (this.isSwinging && this.currentVine) {
            ...
            // Mantenemos el límite inferior que evita que se quede agarrado en el aire vacío
...
function togglePause() {
    ...
    isPaused = !isPaused;
    if (isPaused) { document.getElementById('pause-screen').classList.remove('hidden'); bgMusic.pause(); }
    else { document.getElementById('pause-screen').classList.add('hidden'); bgMusic.play(); }
}

// Event Listeners (Pausa, Controles)
...
canvas.addEventListener('mousedown', () => {
    if(!isGameOver && !isPaused && (player.isSwinging || player.isOnGround)) isPressing = true;
});
...
function updateLoop() {
    ...
    if (isPressing && player.charge < 20) player.charge += 0.5;
    ...
    coinsArray.forEach((coin) => {
        ...
        if (!coin.collected && player.x < coin.x + coin.width && player.x + player.width > coin.x && player.y < coin.y + coin.height && player.y + player.height > coin.y) {
            coin.collected = true; coins += 1; document.getElementById('coins').innerText = coins;
        }
    });
    ...
    vines.forEach((vine) => {
        ...
        vine.draw();
        if (!player.isSwinging && player.vy > 0 && player.x < vine.x + vine.width && player.x + player.width > vine.x && player.y < vine.y + vine.totalHeight && player.y + player.height > vine.y) {
            player.isSwinging = true; player.currentVine = vine; player.vx = 0; player.vy = 0;
            distance += 10; document.getElementById('distance').innerText = distance;
        }
    });
...
document.getElementById('buy-jump').addEventListener('click', () => {
    if (coins >= 10) { coins -= 10; jumpPowerUpgrade += 0.2; document.getElementById('coins').innerText = coins; document.getElementById('alert-text').innerText = "¡Salto mejorado!"; document.getElementById('custom-alert').classList.remove('hidden'); }
    else { document.getElementById('alert-text').innerText = "No tienes monedas."; document.getElementById('custom-alert').classList.remove('hidden'); }
});

document.getElementById('close-alert-x').addEventListener('click', () => document.getElementById('custom-alert').classList.add('hidden'));
document.getElementById('close-alert-ok').addEventListener('click', () => document.getElementById('custom-alert').classList.add('hidden'));
